...
# diffusion regressor need noisy data
# so, we have to sample infinite from train dataset and add noise to it
def cycle(dl):
    while True:
        for data in dl:
            yield data
...
def main(args):
    ...
    # time layer is for diffuse step ff network, can select linear or MLP
    time_layer = 'Linear' if args.time_linear  else 'MLP'

    # can ignore when wandb is not needed
    ...
    # can select utilize same-range group label as input [default : disable_g = True]
    if args.disable_g :
    ...
    # Once you've trained all the data in the train set once, validate it. 
    ...
    # Pre-sample val_t_all by the entire validation set size for fair evaluation
    # Which means noise of validation set is fixed
    ...
    with tqdm(initial = 0, total = epochs) as pbar:
        for i in range(epochs):
            ...
            # Diffuse step scheduler need update every epoch
...
if __name__ == '__main__':
    ...
    parser.add_argument("--do_rate", type=float, default=0.5) # Drop out rate

    ## Group inform as input -----------------------------------
